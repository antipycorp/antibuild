...
func InstallModule(name string, version string, repoURL string, filePrefix string) (*modules.Module, errors.Error) {
	...
	if repoURL == repositories.NoRepositorySpecified {
	...
	for _, rURL := range repoURLs {
		...
		if me, ok := (*repo)[name]; ok {
			...
			installedVersion, err := Install(me, version, filepath.Join(filePrefix, "abm_"+name))
			if err != nil {
...
func Install(me repositories.Entry, version string, targetFile string) (string, errors.Error) {
	targetFile += "@" + version
