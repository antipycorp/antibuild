...
var (
	...
	developCMD = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			configFileDevelopCMD := *cmd.Flags().StringP("config", "c", "config.json",
				"Config file that should be used for building. If not specified will use config.json")
			portDevelopCMD := *cmd.Flags().StringP("port", "p", "8080",
	...
	buildCMD = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			configFileBuildCMD := *cmd.Flags().StringP("config", "c", "config.json",
				"Config file that should be used for building. If not specified will use config.json")
